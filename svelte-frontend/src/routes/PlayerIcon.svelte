<script lang="ts">
    ...
    export let discord_user: DiscordUser;
...
<div class="player-container fixed top-5 w-full flex justify-center items-center z-10">
    <div class="relative bg-gray-700 p-4 rounded-lg flex items-center space-x-4 shadow-lg">
        ...
        <div class="text-white text-lg flex flex-col items-start">
            {#if isAdmin}
                <span class="text-red-500 text-sm font-bold">[ADMIN]</span>
            {/if}
            <p>{discord_user.username}</p>
        </div>
    </div>
</div>
